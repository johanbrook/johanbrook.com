// From https://deno.land/std@0.219.0/datetime/_common.ts?source
...
export class DateTimeFormatterTemporal {
    #format: Format;
    ...
    format(date: Temporal.PlainDateTimeLike): string {
        let string = '';
        ...
        for (const token of this.#format) {
            ...
            switch (type) {
                ...
                case 'dayPeriod': {
                    string += token.value ? (date.hour! >= 12 ? 'PM' : 'AM') : '';
                    break;
                }
                case 'literal': {
                    string += token.value;
