...
mod tests {
    ...
    fn full_render_set() {
        ...
        let result = tpl
            .render(&e, &as_map(serde_json::json!({})), LOC, &[], "[]")
            .unwrap();
        assert_eq!(result, "Hi!");
    }
    ...
    fn full_render_else_if() {
        ...
        assert_eq!(
            tpl.render(&e, &as_map(serde_json::json!({"x": 1})), LOC, &[], "[]")
                .unwrap(),
            "one"
        );
        assert_eq!(
            tpl.render(&e, &as_map(serde_json::json!({"x": 2})), LOC, &[], "[]")
                .unwrap(),
            "two"
        );
        assert_eq!(
            tpl.render(&e, &as_map(serde_json::json!({"x": 3})), LOC, &[], "[]")
                .unwrap(),
            "other"
        );
    }
    ...
    fn full_render_empty_filter_string() {
        ...
        assert_eq!(
            tpl.render(&e, &as_map(serde_json::json!({"name": ""})), LOC, &[], "[]")
                .unwrap(),
            "anonymous"
        );
        assert_eq!(
